...
function printResult(){
	...
	function posit(){
		$.ajax({
			type: "GET",
			cash: false,
			url: "process/locations.php", // process query location
			success: function(xmlResponse){
				 if(xmlResponse){ // if ok
					 var locations = new Array();
					 var speed = 0;
					 var lastspeed = 0;
					 var timediff, lastupdate;
					 $(xmlResponse).find("tracker").each(function(index){
						 ...
						 lastspeed = $(this).find('lastspeed').text();
						 timediff = $(this).find('timediff').text();
						 lastupdate = $(this).find('lastrupdate').text();
					...
					//var vehicle = new google.maps.MarkerImage('images/LorryGreen-32x32.png');
					for (i = 0; i < locations.length; i++) {
						//mapView.getOverlays().remove();
						icoImg = new google.maps.MarkerImage('images/homegardenbusiness.png');
						if(locations[i][3] > 3){
						...
						if(locations[i][5] != 1){
							icoImg = new google.maps.MarkerImage('images/iconblack.png');
							flagIcon_shadow = new google.maps.MarkerImage('images/icon15-shadow.png');
							flagIcon_shadow.anchor = new google.maps.Point(10, 35);
						}
						...
						google.maps.event.addListener(marker[i], 'mouseover', (function(marker, i) {
							...
							var divheight = $('.divbox').height();
							var divwidth = $('.divbox').width();
							$('.divbox').css('left',cursorX-(divwidth+50));
							$('.divbox').css('top',cursorY);
							//$('.divbox').css('left',mousobj.x);
							//$('.divbox').css('bottom',mousobj.y + (divheight-40));
...
function mapcenterbyclient(lat,lng){
