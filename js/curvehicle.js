			...
			if(Nxhr.readyState == 4){
				if(Nxhr.status == 200){
					...
					var tracker = Nxhr.responseXML.getElementsByTagName("tracker");
						 ...
						 //curloc = tracker[0].getElementsByTagName("curspeed")[0].firstChild.nodeValue;
						 $("#vimei").html(name);
						 $("#vlat").html(lat);
						 $("#vlng").html(lng);
						 $("#vspeed").html(speed);
					...
					for(var k=0; k<arLocs.length; k++){
						...
						speedloc = arLocs[k].getElementsByTagName("curspeed")[0].firstChild.nodeValue;
						curspeed[k] = arLocs[k].getElementsByTagName("curspeed")[0].firstChild.nodeValue;
						curdatetiem[k] = arLocs[k].getElementsByTagName("datetime")[0].firstChild.nodeValue;
						//alert(speedloc);
						if(k==0){
							posicon = new google.maps.MarkerImage('images/start.png');
						}else{
							posicon = new google.maps.MarkerImage('images/mapm.png');						
						}
						if(k==arLocs.length-1){
							map.setCenter(new google.maps.LatLng(lat,lng));
						}
						var latlng = new google.maps.LatLng(lat,lng);
						defmarker[k] = new google.maps.Marker({
							position: latlng,
							map: map,
							icon: posicon
						});
						google.maps.event.addListener(defmarker[k], 'click', (function(defmarker, k) {
					        return function(e) {
					        	//alert(curspeed[k]);
								$('#custinfowindow').css('left',cursorX-300);
								$('#custinfowindow').css('top', cursorY);	  
					        	$('#speedstat').html(curspeed[k]);
					        	$('#locstat').html(lat + ', ' + lng);
					        	$('#datestat').html(curdatetiem[k]);
					        	$('#custinfowindow').show();
					        };
					     })(defmarker, k));
						//if( speedloc > 2){
